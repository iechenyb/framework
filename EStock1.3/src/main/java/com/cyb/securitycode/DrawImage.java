...
/** http://blog.csdn.net/kunkun378263/article/details/25644727 
 *  图片处理 包括截图 裁剪  放大 缩小等功能！
...
public class DrawImage extends HttpServlet {
    ...
    public static final int LINENUMBERS = 10;
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (createTypeFlag.equals("ch")) {
            // 截取汉字
        	request.getSession().setAttribute("chcheckcode", random);
        }else if (createTypeFlag.equals("nl")) {
            // 截取数字和字母的组合
        	request.getSession().setAttribute("nlcheckcode", random);
        }else if (createTypeFlag.equals("n")) {
            // 截取数字
        	request.getSession().setAttribute("ncheckcode", random);
        }else if (createTypeFlag.equals("l")) {
            // 截取字母
        	request.getSession().setAttribute("lcheckcode", random);
        }
