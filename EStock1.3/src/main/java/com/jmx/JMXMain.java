...
//      MBeanServer mbs = MBeanServerFactory.createMBeanServer();//不能在jconsole中使用  
        ...
        //创建MBean  
        ControllerMBean controller = new Controller(); 
        Controller1MBean controller1 = new Controller1(); 
        //将MBean注册到MBeanServer中  
        mbs.registerMBean(controller, new ObjectName("MyappMBean1:name=cybBean"));  
        mbs.registerMBean(controller1, new ObjectName("zookeeper:name=zookeeperBean")); 
        //创建适配器，用于能够通过浏览器访问MBean  
       /* HtmlAdaptorServer adapter = new HtmlAdaptorServer();  
        adapter.setPort(9797);  
        mbs.registerMBean(adapter, new ObjectName(  
                "MyappMBean2:name=htmladapter,port=9797"));  
        adapter.start();*/
        ...
        dialog.setName("jmx test");  
        dialog.setVisible(true);
        //http://localhost:9797/  访问 也可以用客户端运行查看
	}
}
/**
 * 1.JMX 简介 
